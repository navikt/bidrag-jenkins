...
class DockerImage {
    ...
    DockerImage(PipelineEnvironment pipelineEnvironment, String username, String password) {
    ...
    void releaseNew() {
        if (pipelineEnvironment.isSnapshot()) {
            ...
            pipelineEnvironment.multibranchPipeline.withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'nexusCredentials', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD']]) {
                pipelineEnvironment.multibranchPipeline.sh "docker login -u ${username} -p ${password} ${pipelineEnvironment.dockerRepo}"
